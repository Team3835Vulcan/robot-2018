...
TrajectoryController::TrajectoryController() :
	m_kp(0), m_ki(0), m_kd(0), m_kv(0), m_ka(0), m_kt(0), m_currTime(0),
...
const DriveSignal TrajectoryController::Calculate(){
	if(m_enabled){
		...
		m_currHeading =  Chassis::GetInstance().GetAngle() + 90; //offset from navx
		...
		double output = m_kp * deltaDist + m_kv * m_goalVel + m_ka * currAccel;
		output = Clamp(output, -1, 1);
		return {output + turn, output - turn};
	}
	return {0,0};
}
