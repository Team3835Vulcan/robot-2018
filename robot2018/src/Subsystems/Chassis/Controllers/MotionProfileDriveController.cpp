...
void MotionProfileDriveController::SetPID(double p, double i, double d){
	frc::SmartDashboard::PutString("hello", "hihi");
	m_velController->SetPID(p,i,d);
}
...
void MotionProfileDriveController::Calculate(){
	if(m_enabled){
		...
		frc::SmartDashboard::PutNumber("time", m_currTime);
		auto goalVel = m_profile->GetSetpoint(m_currTime)->GetVelocity();
		m_velController->SetSetpoint(goalVel);
